  ...
  :i2c(SIOD, SIOC)
{
  ...
  DEBUG_PRINTLN("[CAMERA] XCLK externo: aguardando VSYNC...");
  ...
  while (!digitalRead(VSYNC))
  {
    ...
    {
      DEBUG_PRINTLN("[CAMERA] ERRO: VSYNC nao subiu em 1 segundo.");
      break;
    }
  }
  ...
  {
    DEBUG_PRINTLN("[CAMERA] VSYNC HIGH detectado.");
  }
  ...
  while (digitalRead(VSYNC))
  {
    if (millis() - inicio > 1000)
    {
      DEBUG_PRINTLN("[CAMERA] ERRO: VSYNC nao desceu em 1 segundo.");
      break;
    }
  }

  DEBUG_PRINTLN("[CAMERA] VSYNC LOW detectado.");
  DEBUG_PRINTLN("[CAMERA] Continuando inicializacao.");
